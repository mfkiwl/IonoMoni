...
def get_global_vtec_range(data_folder, sitelists, system):
    ...
    for site in sitelists:
        vtec_file = os.path.join(data_folder, f"{site}_VTEC_{system}.txt")
        if not os.path.exists(vtec_file):
            continue
...
def plot_multi_station_vtec(
    ...
    for hour in range(24):
        ...
        for site in sitelists:
            vtec_file = os.path.join(data_folder, f"{site}_VTEC_{system}.txt")
            lat_file = os.path.join(data_folder, f"{site}_IPP_lat_{system}.txt")
            lon_file = os.path.join(data_folder, f"{site}_IPP_lon_{system}.txt")
            if not (os.path.exists(vtec_file) and os.path.exists(lat_file) and os.path.exists(lon_file)):
                continue
            vtec = load_vtec_txt(vtec_file)
        ...
        if lons_all and lats_all:
            ...
            lat_max = np.ceil(np.max(lats_all)) + 0.5
