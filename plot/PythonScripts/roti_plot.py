...
def plot_roti_or_rot(file_path, save_path, title_prefix="ROTI", max_sat=32, unit="min"):
    ...
    for i in range(data.shape[1]):
        ...
        plt.scatter(x, data[:, i], color=colors(i / data.shape[1]), s=10, label=prn_label, alpha=0.6)
...
def batch_plot_roti_rot(base_input_path, base_output_path, year, doy, rot_unit="min", site_list=None):
    ...
    for file in all_files:
        ...
        prefix_map = {"GPS": "G", "GLO": "R", "BDS": "C", "GAL": "E", "GALX": "E"}
        prefix = prefix_map.get(system, system[0])
        max_sat = {"GPS": 32, "GLO": 24, "BDS": 46, "GAL": 36, "GALX": 36}.get(system, 32)
...
if __name__ == "__main__":
    ...
    site_list = ["HKWS", "ABPO"]  # Add your station names here
