...
void calc_roti_GPS(const obs& OBS, const std::string& stationName, const sp3& SP3, bool isC1WAllZero, const std::string& txt_output_path, t_gcfg_ppp& gset) {
    ...
    // Compute geometry-free linear combination for each satellite and epoch
    for (int i = 1; i <= 32; i++) {
        ...
        // Select ROTI scaling factor based on unit
        if (rot_unit == "sec") {
        ...
        // Calculate ROT (Rate of TEC Index)
...
void calc_roti_GALX(const obs& OBS, const std::string& stationName, const sp3& SP3, const std::string& txt_output_path, t_gcfg_ppp& gset){
    ...
    std::string galx_roti_path = txt_output_path.substr(0, txt_output_path.find_last_of(".")) + "_GALX_ROTI.txt";
    std::string galx_rot_path = txt_output_path.substr(0, txt_output_path.find_last_of(".")) + "_GALX_ROT.txt";

    std::ofstream out(galx_roti_path), rot(galx_rot_path);
    if (!out.is_open() || !rot.is_open()) {
        std::cerr << "Failed to open GALX ROTI/ROT output file." << std::endl;
        return;
    }
