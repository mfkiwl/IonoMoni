...
void clean_short_arcs(std::vector<std::vector<int>>& arcs, obs& OBS, int i, int arc_min_len) {
    // Remove arcs shorter than threshold
...
void R_prepro(obs& OBS, const std::string& stationName, const std::string& dcbFilePath, const std::string& txt_output_path, t_gcfg_ppp& gset, const sp3* SP3, int mf_type) {
    ...
    std::vector<double> satelDCB;
    double recDCB;
    readGLODCB(dcbFilePath, stationName, satelDCB, recDCB);
...
void E_prepro(obs& OBS, const std::string& stationName, const std::string& dcbFilePath, const std::string& txt_output_path, t_gcfg_ppp& gset, const sp3* SP3, int mf_type) {
    ...
    std::vector<double> satelDCB;
    double recDCB;
    readGALDCB(dcbFilePath, stationName, satelDCB, recDCB);
...
void EX_prepro(obs& OBS, const std::string& stationName, const std::string& dcbFilePath, const std::string& txt_output_path, t_gcfg_ppp& gset, const sp3* SP3, int mf_type) {
    double f1 = 1575.42e6, f2 = 1176.45e6;
    double lambda_wl = c / (f1 - f2);
    double MW[37][2881], GF[37][2881], wlAmb[37][2881];
    double P4[37][3000], L4[37][3000], smoothedP4[37][3000];
    ...
    for (int i = 1; i <= 36; i++) {
        ...
        apply_smoothing_filter(smoothedP4, L4, ARCS, i);
        output_smoothed_vtec_txt(
            "result_GALX_VTEC.txt", "E", 36, smoothedP4, f1, f2, OBS, SP3, mf_type,
            [](const sp3* sp3_ptr, int prn, int epoch, double& x, double& y, double& z) {
    ...
    std::vector<double> satelDCB;
    double recDCB;
    readGALXDCB(dcbFilePath, stationName, satelDCB, recDCB);
    apply_dcb_correction_to_matrix(smoothedP4, 36, satelDCB, recDCB);

    std::string out_path = txt_output_path.substr(0, txt_output_path.find_last_of(".")) + "_GALX.txt";
    output_smoothed_stec_txt(out_path, "E", 36, smoothedP4, f1, f2);
    output_smoothed_vtec_txt(
        "result_GAL_VTEC.txt", "E", 36, smoothedP4, f1, f2, OBS, SP3, mf_type,
        [](const sp3* sp3_ptr, int prn, int epoch, double& x, double& y, double& z) {
            x = sp3_ptr[1].EX[prn][epoch];
            y = sp3_ptr[1].EY[prn][epoch];
            z = sp3_ptr[1].EZ[prn][epoch];
...
void C_prepro(obs& OBS, const std::string& stationName, bool isC7IAllZero, const std::string& dcbFilePath, const std::string& txt_output_path, t_gcfg_ppp& gset, const sp3* SP3, int mf_type) {
    ...
    std::vector<double> satelDCB;
    double recDCB;
    readBDSDCB(dcbFilePath, stationName, satelDCB, recDCB, isC7IAllZero);
    ...
    output_smoothed_vtec_txt(
        "result_BDS_VTEC.txt", "C", 46, smoothedP4, f1, f2, OBS, SP3, mf_type,
        [](const sp3* sp3_ptr, int prn, int epoch, double& x, double& y, double& z) {
