...
void G_prepro(
    ...
    for (int i = 1; i <= 32; i++) {
        ...
        // Carrier phase smoothing: supports both "hatch" and "arc_mean" smoothing methods
        if (smoothing == "hatch") {
            // Hatch filter
            apply_hatch_filter(smoothedP4, L4, ARCS, i);
        }
        else if (smoothing == "arc_mean") {
            // Arc mean smoothing
    ...
    if (!readGPSDCB(dcbFilePath, stationName, satelDCB, recDCB, isC1WAllZero, missingPRNs, my_logger)) {
...
void R_prepro(
    ...
    double recDCB;
    std::set<int> missingPRNs;
    if (!readGLODCB(dcbFilePath, stationName, satelDCB, recDCB, missingPRNs, my_logger)) {
        throw std::runtime_error("No DCB record found for station: " + stationName);
    }
    ...
    std::shared_ptr<spdlog::logger> my_logger)
{
    ...
    std::string smoothing = gset.smoothing_method();  // Get smoothing method: "hatch" or "arc_mean"

    for (int i = 1; i <= 36; i++) {
        // Compute MW, GF, and wide-lane ambiguity
        compute_MW_GF_wlAmb(OBS.L1[i], OBS.L2[i], OBS.C1[i], OBS.C2[i], f1, f2, lambda_wl,
            MW[i], GF[i], wlAmb[i]);
        // Detect arcs and filter out short segments
        split_and_filter_arcs(MW[i], i, gset.arc_min_length(), ARCS, OBS);
        // Detect and mark cycle slips
        detect_cycle_slip(wlAmb[i], GF[i], OBS, i, ARCS, gset.arc_min_length(), cs_epoch);
        ...
        if (smoothing == "hatch") {
            ...
            // Default to arc_mean if unrecognized
            smooth_with_arc_mean(smoothedP4, L4, P4, ARCS, i);
        }
    }
    ...
    if (!readGALDCB(dcbFilePath, stationName, satelDCB, recDCB, missingPRNs, my_logger, isC1QAllZero)) {
        throw std::runtime_error("No DCB record found for station: " + stationName);
    }
    apply_dcb_correction_to_matrix(smoothedP4, 36, satelDCB, recDCB);

    // Remove PRNs with missing DCB correction
    for (int prn : missingPRNs) {
        for (int j = 1; j <= 2880; ++j) {
            smoothedP4[prn][j] = 0.0;
        }
...
void C_prepro(obs& OBS,
    ...
    double recDCB;
    std::set<int> missingPRNs;
    if (!readBDSDCB(dcbFilePath, stationName, satelDCB, recDCB, isC7IAllZero, missingPRNs, my_logger)) {
        throw std::runtime_error("No DCB record found for station: " + stationName);
    }
