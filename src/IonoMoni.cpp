    ...
    for (const auto& station : stations)
    {
...

        {
            for (const auto& entry : pppflt->get_sion_memory()) {
                ...
                //my_logger->info("DEBUG-DCB Lookup Keys:");
                //my_logger->info("   sys={}, prn_num={}, station={}", sys, prn_num, station);
                //my_logger->info("   orig_code1={}, orig_code2={}", orig_code1, orig_code2);
                //my_logger->info("   mapped_code1={}, mapped_code2={}", dcb_code1, dcb_code2);
                //my_logger->info("   satKey={}, recKey={}", satKey, recKey);

                //double satDCB = 0.0;
                //if (satelliteDCBMap.find(satKey) != satelliteDCBMap.end())
                //    satDCB = satelliteDCBMap.at(satKey);
                //double recDCB = 0.0;
                //if (receiverDCBMap.find(recKey) != receiverDCBMap.end())
                //    recDCB = receiverDCBMap.at(recKey);

                //double dcb_total_ns = satDCB + recDCB; // ns
                //double dcb_total_m = dcb_total_ns * 1e-9 * c; // 2m
                //value = value + dcb_total_m;
                ...
                //double dcb_total_ns = satDCB + recDCB;
                ...
                //else if (!satDCB_found) {                                               //Waiting for update
                ...
                //}
                    ...
                    if (f1 > 0 && f2 > 0) {
                        double satDCB = satelliteDCBMap.at(satKey);
                        double recDCB = receiverDCBMap.at(recKey);
                        double dcb_total_ns = recDCB;  ////Waiting for update
                        double dcb_total_m = dcb_total_ns * 1e-9 * c;
                        value = value - ((f2 * f2) / (f2 * f2 - f1 * f1)) * dcb_total_m;
                        //my_logger->info("Epoch {} PRN {} code {}: satDCB={} ns, recDCB={} ns, dcb_total_m={} m, corrected_value={}",
                        //    epo.str_ymdhms(), prn, code_type, satDCB, recDCB, dcb_total_m, value);
                    }
