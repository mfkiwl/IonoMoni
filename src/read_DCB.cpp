    ...
    std::shared_ptr<spdlog::logger> my_logger)
{
    ...
    for (int prn = 1; prn <= 32; ++prn) {
        if (!found[prn]) {
            my_logger->warn("Satellite DCB not found for GPS PRN {}.", prn);
    ...
    std::shared_ptr<spdlog::logger> my_logger)
{
    ...
    for (int prn = 1; prn <= 46; ++prn) {
        if (!found[prn]) {
            my_logger->warn("Satellite DCB not found for BDS PRN {}.", prn);
            missingPRNs.insert(prn);
        }
    }
    return foundReceiver;
    ...
    std::shared_ptr<spdlog::logger> my_logger)
{
    ...
    for (int prn = 1; prn <= 24; ++prn) {
        if (!found[prn]) {
            my_logger->warn("Satellite DCB not found for GLO PRN {}.", prn);
            missingPRNs.insert(prn);
        }
    }
    return foundReceiver;
    ...
    bool isC1QAllZero)
{
    ...
    for (int prn = 1; prn <= 36; ++prn) {
        if (!found[prn]) {
            my_logger->warn("Satellite DCB not found for GAL PRN {}.", prn);
            missingPRNs.insert(prn);
        }
    }
    return foundReceiver;
