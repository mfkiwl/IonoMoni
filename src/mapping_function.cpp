...
// Constant definition
const double rad2semi = 1.0 / PI * 180.0 / 0.5; // ¡Ö 114.59
...
double get_mapping_function(double e, int mf_type)
{
    ...
    case 1: // MSLM (Modified Single-Layer Model)
    {
        double z = PI / 2 - e;
        double alpha = 0.9782;
        return 1.0 / sqrt(1 - pow(Re * sin(alpha * z) / (Re + 506700.0), 2));
    }
    // case 2: // KLOBUCHAR
    // {
    ...
    case 2: // Foster-Knight (F&K) mapping function
    {
        double a = 1 + (Re + h) / Re;
        double b = sin(e) + sqrt(pow((Re + h) / Re, 2) - pow(cos(e), 2));
        return a / b;
    }
    case 3: // Ouyang et al. mapping function
    {
        double z = PI / 2 - e;
        double z_pie = asin(Re * sin(z) / (Re + h));
