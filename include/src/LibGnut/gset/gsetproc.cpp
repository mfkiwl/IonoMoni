...
namespace gnut
{
    ...
    int t_gsetproc::roti_window()
    {
        _gmutex.lock();
        string tmp = _doc.child(XMLKEY_ROOT).child(XMLKEY_PROC).child_value("roti_window");
        str_erase(tmp);
        int value = tmp.empty() ? 10 : std::stoi(tmp); 
        _gmutex.unlock();
        return value;
    }

    std::string t_gsetproc::rot_unit()
    {
        _gmutex.lock();
        string tmp = _doc.child(XMLKEY_ROOT).child(XMLKEY_PROC).child_value("rot_unit");
        str_erase(tmp);
        std::transform(tmp.begin(), tmp.end(), tmp.begin(), ::tolower);
        if (tmp != "sec" && tmp != "min") tmp = "min";  
        _gmutex.unlock();
        return tmp;
    }

    std::string t_gsetproc::smoothing_method()
    {
        _gmutex.lock();
        string tmp = _doc.child(XMLKEY_ROOT).child(XMLKEY_PROC).child_value("smoothing");
        str_erase(tmp);
        std::transform(tmp.begin(), tmp.end(), tmp.begin(), ::tolower); 
        if (tmp.empty()) tmp = "arc_mean"; 
        _gmutex.unlock();
        return tmp;
    }


    int t_gsetproc::aatr_interval()
    {
        _gmutex.lock();
        string tmp = _doc.child(XMLKEY_ROOT).child(XMLKEY_PROC).child_value("aatr_interval");
        str_erase(tmp);
        int value = tmp.empty() ? 120 : std::stoi(tmp); 
        _gmutex.unlock();
        return value;
    }

    int t_gsetproc::arc_min_length()
    {
        _gmutex.lock();
        string tmp = _doc.child(XMLKEY_ROOT).child(XMLKEY_PROC).child_value("arc_min_length");
        str_erase(tmp);
        int value = tmp.empty() ? 10 : std::stoi(tmp);  
        _gmutex.unlock();
        return value;
    ...
    double t_gsetproc::ion_height()
    {
        _gmutex.lock();
        string tmp = _doc.child(XMLKEY_ROOT).child(XMLKEY_PROC).child_value("ion_height");
        str_erase(tmp);
        double value = tmp.empty() ? 350000.0 : std::stod(tmp);  // 默认350000米
        _gmutex.unlock();
        return value;
    }

    int t_gsetproc::mapping_function()
    {
        _gmutex.lock();
        string tmp = _doc.child(XMLKEY_ROOT).child(XMLKEY_PROC).child_value("mapping_function");
        str_erase(tmp);
        int value = tmp.empty() ? 0 : std::stoi(tmp);  // 默认用 SLM（编号0）
        _gmutex.unlock();
        return value;
    }
