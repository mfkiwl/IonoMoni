...
namespace great
{
    ...
    */
    ...
    public:
        // --- 获取SION缓存 ---
        const std::vector<std::tuple<t_gtime, std::string, par_type, double>>& getSionMemory() const {
            return _sion_memory;
        }
        ...
        */
        ...
        // 提供SION结果给外部调用
        const vector<std::tuple<t_gtime, std::string, par_type, double>>& get_sion_memory() const { return _sion_memory; }
    ...
    protected:
        ...
        // 保存每个历元的SION参数到内存
        vector<std::tuple<t_gtime, std::string, par_type, double>> _sion_memory;
