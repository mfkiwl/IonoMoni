...
int great::t_gpvtflt::_processEpoch(const t_gtime &runEpoch)
{
    ...
    vector<t_gpar> allParams = _param.getAllPar();

    // _processEpoch  prn2code_this_epoch
    //  prn2code_this_epoch
    std::map<std::string, std::string> prn2code_this_epoch;
    for (const auto& iter : _data) {
        std::string prn = iter.sat();
        GSYS gs = iter.gsys();
        GOBSBAND b1 = _band_index[gs][FREQ_1];
        GOBSBAND b2 = _band_index[gs][FREQ_2];

        std::string code_used = gobs2str(iter.select_range(b1)) + "," + gobs2str(iter.select_range(b2));
        prn2code_this_epoch[prn] = code_used;
    }

    // save sion_memory
    for (const auto& param : allParams) {
        if (param.parType == par_type::SION) {
            std::string code_used = "";
            auto it = prn2code_this_epoch.find(param.prn);
            if (it != prn2code_this_epoch.end())
                code_used = it->second;
            _sion_memory.push_back(std::make_tuple(runEpoch, param.prn, param.parType, param.value(), code_used));
        }
    }
...
bool great::t_gpvtflt::_check_sat(const string& ssite, t_gsatdata* const iter, Matrix& BB, int& iobs)
{
    ...
    //std::ofstream out("prn_code_used.txt", std::ios::app);
    //out << _epoch.str_ymdhms() << "," << iter->sat() << ","
    //    << gobs2str(p1) << "," << gobs2str(p2) << std::endl;
    //out.close();
